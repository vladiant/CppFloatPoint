...
auto calcDotProductKahanAlg(const std::vector<T>& v, const std::vector<T>& w) {
    ...
    for (size_t i = 0; i < kElems; i++) {
        ...
        const T t =
            theSum + y;  // Add corrected summand to the running sum theSum
...
auto calcDotProductStdAlgPar(const std::vector<T>& v, const std::vector<T>& w) {
    return std::transform_reduce(std::execution::par, std::begin(v),
                                 std::end(v), std::begin(w), T{});
}
...
auto calcDotProductSortAlgPar(const std::vector<T>& v,
...

    {
        TimeInterval t{"  sort       "};
        // Sort in descending order
        std::sort(std::execution::par, std::begin(z), std::end(z),
                  [](const auto& lhs, const auto& rhs) {
                      return abs(lhs) < abs(rhs);
                  });
    }
